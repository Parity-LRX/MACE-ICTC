"""Load a real e3nn MACE (``mace-torch``) model into a MACE-ICTD model.
...
:class:`mace_ictd.models.pure_cartesian_ictd_fix.PureCartesianICTDFix`. The exact
default/recommended target is ``ictd_fix_product_backend="ictd-bridge-u"``, which uses
the MACE U tensors folded into the ICTD basis. ``native-mace`` is also supported as a
...
1. **e3nn ``o3.Linear`` blocks** (``node_embedding``, ``linear_up``, the post-conv
   ...
   exactly ``M_l ⊗ I_(2l+1)`` in the channel-major ``(c, m)`` layout that ICTD also uses,
   so ``M_l`` drops straight into the ICTD adapter (``nn.Linear`` with ``weight = M_l.T``).
   This is exact to machine precision and version-independent.

2. **The convolution tensor product** (MACE ``conv_tp`` ↔ ICTD ``tp``). Both evaluate the
   same ``(l1, l2) -> l3`` paths but (a) in a different path order and (b) with a different
   per-path scalar (e3nn's element path-normalisation, which ICTD's CG basis does not
   ...
   ICTD's ``tp`` and the per-path scalar ``c_p`` is recovered by least squares. The
   MACE radial MLP (``conv_tp_weights``) is then copied into ICTD's ``fc`` with the row for
   path ``p`` scaled by ``c_p`` and reordered into ICTD's path order. ICTD's own per-path
...
3. **The symmetric contraction** uses the bridge-U path with MACE U tensors pre-folded
   into the ICTD basis by default. The literal MACE code path
...
per ``l`` (ICTD vs e3nn), but energy/forces are SO(3) invariants and therefore identical;
...
from mace_ictd.models.ictd_irreps import direction_harmonics_all
...
# --------------------------------------------------------------------------------------
# helpers: e3nn o3.Linear -> ICTD per-l effective channel matrices
# --------------------------------------------------------------------------------------
...
def _copy_into_so3_channel_linear(adapters_module, mats: Dict[int, torch.Tensor]) -> None:
    ...
    ``nn.Linear(C_in, C_out, bias=False)``. The ICTD adapter computes ``y = x @ W.T`` over the
...
def _calibrate_conv_tp(
    ...
    *,
    ...
    """Determine, for the MACE ``conv_tp`` / ICTD ``tp`` pair:

    - ``perm``: for each MACE path index, the corresponding ICTD path index.
    ...
    MACE ``conv_tp`` (with random per-path channel weights) and ICTD ``tp`` (gates = those
    ...
    N = int(n_probe)
    # node features (ICTD basis) up to input_lmax; zero above
    h_ictd = {
        l: (
...
def convert_mace_to_ictd(mace_model, ictd_model) -> Dict[str, object]:
    ...
    with torch.no_grad():
        # ICTD node_embedding is nn.Linear(num_elements, C): out = onehot @ W.T => W = M0.T
    ...
    for i, (m_inter, ictd_inter) in enumerate(zip(mace_model.interactions, ictd_model.interactions)):
        ...
        # linear_up: e3nn Linear (in irreps over input_lmax) -> ICTD linear_up adapters
        ...
        # ICTD per-path base weight has no MACE equivalent -> all ones (path weight = fc alone)
        with torch.no_grad():
            ictd_inter.tp.weight.fill_(1.0)

        # radial MLP: MACE conv_tp_weights (e3nn FullyConnectedNet, no bias) -> ICTD fc.
        #
        ...
        # ICTD fc is [Linear, SiLU, Linear, SiLU, Linear, SiLU, Linear] (plain SiLU, bias).
        #
        ...
        # activated ICTD layer's output a factor 1/K below e3nn; (2) fold that 1/K into the
        ...
        with torch.no_grad():
            ...
            new_last = torch.zeros_like(ictd_fc_linears[3].weight)  # (P*C, 64) ICTD path order
        ...
        # width (count_l3 * C) to C. ICTD message_linear is a PathPreservingLinearSO3 with the
        ...
        # MACE expands paths-to-l3 in conv_tp instruction order; ICTD orders them by path_offset.
        # Reorder the input-channel axis of each M_l from MACE order to ICTD order before copy.
        ...
        report["blocks"][f"interaction[{i}].message_linear"] = (
            "MACE post-conv linear effective-matrix, input paths reordered MACE->ICTD"
        )
        ...
        # ICTD layers > 0 own an additive `self_connection` -> direct l=0 per-element copy.
        # The FIRST ICTD interaction has no additive sc path (`self_connection=None`,
        ...
        if ictd_inter.self_connection is not None:
            ...
            if m_inter.__class__.__name__ == "RealAgnosticInteractionBlock":
                ...
                report["blocks"][f"interaction[{i}].message_selector"] = (
                    ...
                    "ICTD baseline's first interaction omits) injected as an additive per-element bias "
    ...
    if len(ictd_model.layer_energy_readouts) != int(ictd_model.num_interaction) - 1:
        raise RuntimeError("ICTD intermediate readout count does not match num_interaction")
    ...
    report["blocks"]["scale_shift"] = (
        "MACE ScaleShiftBlock copied into ICTD energy_output_scale/energy_output_shift buffers"
    )
...
def _reorder_linear_input_paths(
    ...
    *,
    ...
    ICTD path order. MACE's post-conv linear sees the expanded irreps in conv_tp instruction
    order (paths to a given l3 concatenated in that order); ICTD's message_linear sees them in
    ...
    for l in range(target_lmax + 1):
        ...
        # ICTD target order: by path_offset
...
def _copy_skip_tp_l0(skip_tp, self_connection, *, num_elements: int, channels: int) -> None:
    ...
    ``self_connection`` (l=0 only). ICTD computes ``out[c'] = sum_c W[e, c', c] x[c]`` so
    ``W[e] = M[e].T``."""
...
def _install_first_layer_sc0(ictd_model, *, layer_idx: int, sc0_by_element: torch.Tensor) -> None:
    """Inject MACE's first-layer element self-connection that the ICTD baseline's first interaction
    ...
    per-element constant in the l=0 channels. The ICTD baseline's ``product[0]`` is called with
    ...
    if not hasattr(ictd_model, "install_mace_first_layer_sc0"):
        raise TypeError("target ICTD model does not expose install_mace_first_layer_sc0()")
...
def _install_energy_scale_shift(ictd_model, *, scale: float, shift: float) -> None:
    """Install MACE ScaleShiftMACE buffers into a converted ICTD model."""
    try:
