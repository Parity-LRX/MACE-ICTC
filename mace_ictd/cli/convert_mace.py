"""Convert a mace-torch ScaleShiftMACE model into a MACE-ICTD checkpoint.

Example:
    python -m mace_ictd.cli.convert_mace --mace-model mace.model --out mace_ictd.pth
"""
...
def _infer_uniform_hidden(hidden_irreps: o3.Irreps) -> tuple[int, int]:
    ...
    if missing:
        raise ValueError(
            f"unsupported hidden_irreps={hidden_irreps}; MACE-ICTD conversion needs contiguous l=0..L"
        )
...
def _validate_supported_config(cfg: dict[str, Any], *, channels: int, lmax: int) -> None:
    ...
    if bool(cfg.get("pair_repulsion", False)):
        raise ValueError("pair_repulsion=True is not supported by the MACE-ICTD converter")
...
def convert_model(
    ...
    *,
    ...
    ckpt = {
        "format": "mace-ictd-converted-from-mace-torch",
